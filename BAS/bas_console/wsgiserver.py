    ...
    def communicate(self):
        ...
        try:
            while True:
                ...
                request_seen = True
                import cProfile
                cProfile.runctx("req.respond()", globals(), locals(), filename="test_function.cprof")
                '''
                req.respond() #comment this for profiling...
                if req.close_connection:
                    return
        except socket.error, e:
            errnum = e.args[0]
...
class CheckThread(threading.Thread):
    ...
    def run(self):
        ...
        get_queue_size = self.thread_pool._queue.qsize
...
class ThreadPool(object):
    ...
    def __init__(self, server, min=2, max=20):
        self.server = server
        self.min = min
        self.max = max
        self._threads = []
...
class HTTPServer(object):
    ...
    def __init__(self, bind_addr, gateway, minthreads=2, maxthreads=10,
                 server_name=None):
...
class CherryPyWSGIServer(HTTPServer):
    ...
    def __init__(self, bind_addr, wsgi_app, maxthreads=1, server_name=None,
                 request_queue_size=5, timeout=10, shutdown_timeout=5):
        self.requests = ThreadPool(self, min=5, max=maxthreads)
