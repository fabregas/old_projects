...
import inspect
from soaplib.xmlutils import ns, create_xml_element, create_xml_subelement
...
class ClassSerializerMeta(type):
    ...
    def __init__(cls, clsname, bases, dictionary):
        ...
        for k, v in members.items():
            ...
            elif hasattr(v,'__class__') and issubclass(v.__class__, Array):
                soap_members[k] = v

        cls.soap_members = soap_members
...
class ClassSerializer(object):
    ...
    def __init__(self, **kwargs):
        ...
        for k, v in cls.soap_members.items():
            ...
            if arg is not None:
                setattr(self, k, arg)
            elif inspect.isclass(v) and issubclass(v, ClassSerializer):
                setattr(self, k, v() )
            else:
                setattr(self,k,None)
