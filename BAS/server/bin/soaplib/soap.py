...
from soaplib.xmlutils import create_xml_element, create_xml_subelement
from soaplib.xmlutils import NamespaceLookup, ElementTree
...
def from_soap(xml_string):
    ...
    #root = ElementTree.XML(xml_string.encode(), parser=ElementTree.ETCompatXMLParser()) #FIXME!!!!!!!!!!!
    if xmlids:
...
def join_attachment(id, envelope, payload, prefix=True):
    ...
    soaptree = ElementTree.parse(soapmsg)#, parser=ElementTree.ETCompatXMLParser())
