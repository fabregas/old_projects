...
@package blik.management.core.models
@author Konstantin Andrusenko
@date August 12, 2011

This module contains the models of database's tables.
This models should be used for all operations with data that stored in database
"""
...
class NmClusterTypeNodes(models.Model):
    id = models.AutoField(primary_key=True)
    cluster_type = models.ForeignKey(NmClusterType, db_column='cluster_type')
    node_type = models.ForeignKey(NmNodeType, db_column='node_type')

    class Meta:
        db_table = u'nm_cluster_type_nodes'
...
class NmNode(models.Model):
    ...
    admin_status = models.SmallIntegerField(default=1)
    current_state = models.SmallIntegerField(default=0)
    last_datestart = models.DateTimeField(null=True)
    login = models.CharField(max_length=50)
    password = models.CharField(max_length=50)
    last_modifier_id = models.BigIntegerField()
    hw_info = models.TextField(null=True)
    sw_info = models.TextField(null=True)
    dc = models.DateTimeField(null=True)
    dm = models.DateTimeField(null=True)
